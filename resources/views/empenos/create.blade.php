<x-layouts::app :title="'Nuevo empeño'">
        ...
        <div class="grid gap-5 lg:grid-cols-3">
            <form method="POST" action="{{ route('empenos.store') }}" id="empForm"
                ...
                <div>
                    ...
                    <select name="categoria" id="e_cat" onchange="renderAttrs()" class="w-full rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 text-sm dark:border-zinc-600 dark:bg-zinc-800 dark:text-white"></select>
                </div>
            ...
            <div class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div class="mt-3 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 text-xs text-zinc-500 dark:border-zinc-700 dark:bg-zinc-800/60">
                    El <b>préstamo sugerido</b> es un aproximado: usa categoría, atributos y <b>año</b> (deprecia por antigüedad). Ajústalo si quieres.
                </div>
            </div>
        </div>
    </div>
    ...
    (function () {
        var CATS = {
            "Moto": [
                { k: "marca", label: "Marca", opts: ["Yamaha", "Honda", "Bajaj", "AKT", "Suzuki", "Kawasaki", "Hero", "KTM", "TVS", "Auteco", "Otra"] },
            ...
            "Celular": [
                { k: "marca", label: "Marca", opts: ["Apple (iPhone)", "Samsung", "Xiaomi", "Motorola", "Huawei", "Oppo", "Realme", "Honor", "Otra"] },
            ...
            "Televisor": [
                { k: "marca", label: "Marca", opts: ["LG", "Samsung", "Sony", "Kalley", "Challenger", "Hyundai", "TCL", "Otra"] },
                { k: "pulgadas", label: "Pulgadas", opts: ["24", "32", "40", "43", "50", "55", "65", "75"] },
                { k: "resolucion", label: "Resolución", opts: ["HD", "Full HD", "4K UHD"] },
                { k: "anio", label: "Año", tipo: "year" }
            ],
            "Computador": [
                { k: "marca", label: "Marca", opts: ["Apple", "HP", "Lenovo", "Dell", "Asus", "Acer", "Otra"] },
            ...
            "Electrodoméstico": [
                { k: "tipo", label: "Tipo", opts: ["Nevera", "Lavadora", "Microondas", "Estufa", "Licuadora", "Otro"] },
                { k: "marca", label: "Marca", opts: ["LG", "Samsung", "Haceb", "Mabe", "Whirlpool", "Kalley", "Otra"] },
                { k: "anio", label: "Año", tipo: "year" }
            ],
            "Joya / Oro": [
            ...
            "Herramienta": [
                { k: "tipo", label: "Tipo" },
                { k: "marca", label: "Marca", opts: ["Bosch", "DeWalt", "Makita", "Truper", "Stanley", "Otra"] },
                { k: "anio", label: "Año", tipo: "year" }
            ],
        ...
        function field(f) {
            var inner;
            if (f.tipo === 'year') {
                var yn = new Date().getFullYear(), o = '<option value="">— elegir —</option>';
                for (var y = yn; y >= yn - 25; y--) o += '<option>' + y + '</option>';
                o += '<option>Anterior</option>';
                inner = '<select id="at_' + f.k + '" name="atributos[' + f.k + ']" onchange="recalcSug()" class="' + input + '">' + o + '</select>';
            } else if (f.opts) {
                inner = '<select id="at_' + f.k + '" name="atributos[' + f.k + ']" onchange="recalcSug()" class="' + input + '"><option value="">— elegir —</option>' +
                    f.opts.map(function (op) { return '<option>' + op + '</option>'; }).join('') + '</select>';
            } else {
        ...
        window.renderAttrs = function () {
            var cat = document.getElementById('e_cat').value;
            var campos = CATS[cat] || [];
            var rows = campos.map(field);
            var html = '';
        ...
        function readAttrs(cat) {
            ...
            (CATS[cat] || []).forEach(function (f) {
                var el = document.getElementById('at_' + f.k);
                if (el && el.value) a[f.k] = el.value;
            });
            return a;
        }
        ...
        function estimarValor(cat, a) {
            ...
            else if (cat === "Celular") { var m = { "Apple (iPhone)": 2600000, "Samsung": 1600000, "Xiaomi": 950000, "Motorola": 850000, "Huawei": 900000, "Oppo": 800000, "Realme": 800000, "Honor": 800000, "Otra": 700000 }; var cp = { "32 GB": .85, "64 GB": .95, "128 GB": 1, "256 GB": 1.15, "512 GB": 1.3 }; v = (m[a.marca] || 0) * (cp[a.capacidad] || 1); }
            else if (cat === "Computador") { var mm = { "Apple": 3500000, "HP": 1600000, "Lenovo": 1600000, "Dell": 1700000, "Asus": 1600000, "Acer": 1400000, "Otra": 1200000 }; v = (mm[a.marca] || 0) * (a.tipo === "Escritorio" ? .9 : 1); }
        ...
        function init() {
            ...
            if (!sel) return;
            sel.innerHTML = Object.keys(CATS).map(function (k) { return '<option value="' + k + '">' + k + '</option>'; }).join('');
            renderAttrs();
